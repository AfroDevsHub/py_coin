...
class Block(Base):
    ...
    block_id: uuid | Column[uuid] = Column(
        "block_id", UUID(as_uuid=True), nullable=False, default=uuid4, unique=True
    )
    ...
    previous_block_id: uuid | Column[uuid] = Column(
        "previous_block_id", UUID(as_uuid=True), nullable=True, unique=True
    )
    next_block_id: uuid | Column[uuid] = Column(
        "next_block_id", UUID(as_uuid=True), nullable=True, unique=True
    )
